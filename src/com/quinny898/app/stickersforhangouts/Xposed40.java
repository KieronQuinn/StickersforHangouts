...
public class Xposed40 {
	...
	protected static Object conversationActivity;
	...
	public static void handleLoadPackage(final LoadPackageParam lpparam)
		 ...
		 * IDs. Search for the ArrayAdapter class in the root, and find the file
		 * with a method with three booleans. Update the adding method below
		 * 
		 ...
		 * textview's text is got
		 * Modify the hook's name
		 * Open ConversationFragment and update the object name for the listview that
		 * is set from the class with casts ["(ListView)"]
		 * 
		 * Update the edit fab hook by searching for "onCreateView" in the root and finding the one that references "PhotoViewFragment"
		 * Update the hooks
		 * 
		 * You are done :D
		 */
		...
		// Add an item
		findAndHookMethod("che", lpparam.classLoader, "a", boolean.class,
				boolean.class, boolean.class, new XC_MethodHook() {
					...
					protected void afterHookedMethod(MethodHookParam param)
						...
						XposedHelpers.callMethod(param.thisObject, "a",
								"Add sticker", mButtonPosition, 2130838666);
						XposedHelpers.callMethod(param.thisObject, "a",
								"Add Drawing", mButtonPosition + 1, 2130838662);
					}
				});
		// Easter eggs
		findAndHookMethod(
				"com.google.android.apps.hangouts.views.MessageListItemView",
				lpparam.classLoader, "d", new XC_MethodHook() {
					...
					protected void afterHookedMethod(MethodHookParam param)
						...
						if (message.trim().equals("/bikeshed")) {
							...
							ListView w = (ListView) getObjectField(
						...
						if (message.trim().equals("/uuddlrlrba")) {
							ListView w = (ListView) getObjectField(
									conversationActivity, "g");
							w.setBackgroundResource(Xposed.mFakeId);
						}
		...
		findAndHookMethod("aev", lpparam.classLoader, "onClick",
					...
					protected void beforeHookedMethod(MethodHookParam param)
						...
						if (Integer.parseInt(param.args[1].toString()) == mButtonPosition) {
							Class<?> composeMessageViewClass = findClass(
									"com.google.android.apps.hangouts.conversation.v2.ComposeMessageView",
									lpparam.classLoader);
		...
		findAndHookMethod("un", lpparam.classLoader, "onCreateView",
								...
								a.runOnUiThread(new Runnable() {
									public void run() {
										...
										RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
												dip, dip);
										params.setMargins(0, 0, dip / 4,
												Xposed.getNavBarHeight(a, margin));
										params.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
										params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
										f.setLayoutParams(params);
										f.setFabColor(Color.WHITE);
										f.setFabDrawable(a.getResources()
												.getDrawable(Xposed.mFakeIdE));
										f.setOnClickListener(new OnClickListener() {
											...
											public void onClick(View arg0) {
												final ProgressDialog pd = new ProgressDialog(
														a);
												pd.setMessage(a
														.getString(Xposed.mFakeIdC));
												pd.setCancelable(false);
												pd.show();
												ImageView h = (ImageView) getObjectField(
														param.thisObject, "h");
												BitmapDrawable input = (BitmapDrawable) h
														.getDrawable();
												Bitmap bitmap = ((BitmapDrawable) input)
														.getBitmap();
		...
		findAndHookMethod("aff", lpparam.classLoader, "onCreateView",
				LayoutInflater.class, ViewGroup.class, Bundle.class,
				new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
							throws Throwable {
						conversationActivity = param.thisObject;
